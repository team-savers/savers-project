...
- `shelter_safety_unified_registry_dedup_guard_mock.csv`: 위 통합 목업(51행) +
  ...
  U로 넘어가는 경우를 재현한다 — `run_pu_pipeline_from_unified_registry()`의
  dedup 전후 대피소 카운트 보존 가드가 실제로 발동하는지 확인하는 테스트 전용
  (PR #60 리뷰). 원본 51행 목업에 바로 섞으면 이 가드가 그 파일을 쓰는 모든
  기존 테스트에서 상시 발동해버리므로 파일을 분리했다.
