...
조인이 아예 불필요했다(팀 확정, 재검토 대상 아님). 이 실제 파일 전용 상수는 아래
...
# 피처를 한 행에 갖고 있는 완성된 피처테이블**이라 조인이 필요 없다 (팀 확정 결정,
# 2026-08-05 — 1차 후보군/shelter_candidates_spatial_*.csv 파일은 공간 피처가 있지만
# 이번 스코프에서는 쓰지 않는다: 건축물대장 쪽에 애초에 좌표가 없어 조인 대상 자체가
# 없다). 아래 `UNIFIED_REGISTRY_*` 상수는 이 실제 파일 전용이며, `loading.py`의
...
# 학습 피처 — 사용자 확정 목록. 전부 이미 숫자형(float64/int64), 결측 존재해도
# LightGBM이 기본 처리하므로 드롭하지 않는다. 주용도코드/구조코드는 카테고리값이라
# `category` dtype 캐스팅이 필요해 이번 스코프에서 제외(후속 과제) — 내진능력은
# 결측 92.3%라 제외(2026-08-05 확정, 재검토 대상 아님).
