...
//
// 포함 화면 5개(2026-08-04 팀장 요청 + 이후 추가). 아래 번호는 SCREENS
// 배열의 실제 순서와 일치시켜 둔다 — 화면을 추가·재배열하면 이 목록도 같은
// 커밋에서 고칠 것(PR #54 리뷰: 개수가 3개로 남아 있고 순서도 배열과
// 달랐다. 이 저장소는 주석을 설계 기록으로 취급한다):
//   1. 잠금화면 웹 푸시 알림 — 팀 채팅(2026-08-03, YKG/도혁)에서 확정한
...
//   2. 우리 동네 위험도 — 개념안(2026-08-04, uibowl.io "AMOU" 지역별
...
//      예시 숫자이며, 배지로 계속 밝힌다.
//   3. 보호자 현황 대시보드 — 개념안. GuardianStatus 계약 필드 형태를
...
//      오인되면 안 된다.
//   4. AI 챗봇 Q&A — ChatDock이 실제로 구현한 «근거 없으면 답하지 않는다»
//      원칙(components/ChatDock.tsx 참고)을 스크립트된 대화로 보여준다.
//      입력창은 disabled — 아무 데도 전송하지 않는다.
//   5. 관리자(Ops) 요약 카드 — 지금 Ops.tsx 현황 탭(17열 표)을 카드 4개 +
...
// 'all' — SCREENS 전체를 세로로 쭉 이어 붙여 스크롤 한 번으로 비교해서
// 본다(팀장 피드백 — 탭으로 하나씩 넘기면 직관적으로 비교하기 힘들다).
// 개수를 여기 적지 않는다 — 화면이 늘 때마다 어긋나는 숫자였다(PR #54 리뷰).
...
export function Mockup() {
  ...
  // 한 화면씩 보기의 기본값은 탭 순서의 첫 번째로 맞춘다 — 키를 직접 적어두면
  // (이전 값 'guardian') 탭 순서가 바뀌었을 때 "한 화면씩 보기"를 누르는
  // 순간 3번째 탭이 선택된 상태로 열린다(PR #54 리뷰).
  const [screen, setScreen] = useState<MockupScreen>(SCREENS[0].key)
  return (
    <main
      style={{
        width: '100%',
