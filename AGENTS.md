...
| `docs/공유_문서/` | Continuously revised working documents (기획서, 인터뷰 가이드, …). Its `README.md` is the index — every file listed with 최종 수정일 · 버전 · 핵심 내용 · 상태·근거, updated in the same PR as the file itself |
...
- **`docs/adr/` is the single source of truth for "확정" (confirmed).** An item that cannot be traced to an ADR number is **검토 중** (under review), not 확정 — this applies to everyone, no exceptions. Any document meant for outside readers (역할_가이드, 공통_가이드, etc.) marks each confirmed item with a one-line source (ADR number or file path); an item with no source line is a signal it hasn't actually been decided yet. If something needs to be promoted to 확정, flag it to the Tech Lead to write the ADR — verbal agreement is not a contract in this repo (see the `Contracts before implementation` rule above), so writing it down is the Tech Lead's job, not the reporter's.
